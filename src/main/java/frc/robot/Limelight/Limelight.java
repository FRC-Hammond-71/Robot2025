...
//         {
//             // We need to rotate the gyro back to Red being zero degrees for limelight MegaTag 2!
//             robotGyro = robotGyro.rotateBy(Rotation2d.fromDegrees(180));
//         }
//         LimelightHelpers.SetRobotOrientation(this.name, robotGyro.getDegrees(), 0, 0, 0, 0, 0);
...
//         // Outlier rejection based on distance and velocity
//         if (lastFusedPose != null) {
//             double distance = lastFusedPose.getTranslation().getDistance(estimatedPose.getTranslation());
//             double velocity = distance / Robot.kDefaultPeriod;
    
//             if (distance > MAX_DISTANCE_PER_CYCLE || velocity > MAX_VELOCITY_CHANGE_PER_CYCLE) 
//             {
//                 SmartDashboard.putBoolean("ll_DistanceRejection", true);
//                 // Reject the estimate if it does not match expected movement!
//                 lastFusedPose = rPose;
//                 return Optional.empty(); 
//             }
//             SmartDashboard.putBoolean("ll_DistanceRejection", false);

    
//             double rotationChange = Math.abs(estimatedPose.getRotation().getRadians() - lastFusedPose.getRotation().getRadians());
//             if (rotationChange > MAX_ROTATION_CHANGE_PER_CYCLE) {
//                 SmartDashboard.putBoolean("ll_RotationRejection", true);
//                 // Reject if rotation change is too large!
//                 lastFusedPose = rPose;
//                 return Optional.empty();
//             }
//             SmartDashboard.putBoolean("ll_RotationRejection", false);
//         }
    
//         // https://docs.wpilib.org/en/stable/docs/software/advanced-controls/filters/linear-filter.html#movingaverage
//         double filteredX = xFilter.calculate(estimatedPose.getX());
//         double filteredY = yFilter.calculate(estimatedPose.getY());
//         double filteredRotation = rotationFilter.calculate(estimatedPose.getRotation().getRadians());
//         filteredRotation = Math.atan2(Math.sin(filteredRotation), Math.cos(filteredRotation));
